<template>
    <div>
        <PageTitle title="设备消费统计"></PageTitle>
        <div class="pageWrapper">
            <div class="lookWrapper">
                <div class="searchWrapper">
                    ...
                    <el-button type="primary" size="medium" class="addBtn" icon="el-icon-tickets" @click="excel" v-if="hasPermission('admin/devicestatistic', 'read')">导出Excel</el-button>
                </div>
                <el-table
                        ref="table"
                        :data="tableData"
                    ...
                    <el-table-column
                            prop="seven_total_money"
                            label="7天总营业额(元)">
                    </el-table-column>
                    <el-table-column
                            prop="thirty_total_money"
                            label="30天总营业额(元)">
                    </el-table-column>
                    <el-table-column
                            prop="range_total_money"
                            label="时间段内总营业额(元)">
                    </el-table-column>
                    <el-table-column
                            prop="range_total_money_discount"
                            label="时间段内总销售提成(元)">
                    </el-table-column>
                </el-table>
                <el-pagination
    ...
    export default {
        ...
        methods:{
            ...
            lists(){
                ...
                checkToken(function () {
                    that.loading = true;
                    that.axios.get('/devices/statistic', {
                        params: { //请求参数
            ...
            excel(){
                let url = this.baseURL + "devices/statisticexcel?search=" + this.searchkey + '&type=' + this.searchType + '&brand=' + this.searchBrand + '&uid=' + this.searchUid + '&start=' + (this.dateTime ? this.dateTime[0] : '') + '&end=' + (this.dateTime ? this.dateTime[1] : '') + '&token=' + this.token.value + '&path=' + encodeURIComponent(aesencode('admin/devicestatistic')) + '&privilege=' + encodeURIComponent(aesencode('read'));
                window.open(url);
            }
        },
        components: {
